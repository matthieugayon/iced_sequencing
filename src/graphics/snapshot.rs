use iced_graphics::{defaults, Backend, Primitive, Renderer};
use iced_native::{mouse, Background, Color, Element, Layout, Point, Rectangle, Size};

use crate::native::snapshot;
pub use crate::style::snapshot::{Default, Style, StyleSheet};

pub type Snapshot<'a, Message, Backend> = snapshot::Snapshot<'a, Message, Renderer<Backend>>;

use crate::core::grid::{GridEvent, GridPattern};
    ...
    B: Backend,
{
    ...
    fn draw<Message>(
        ...
        let mut primitives: Vec<Primitive> = (0..=grid)
            .into_iter()
            .map(|step| {
                let color = {
                    if step == 0 || step == grid {
        ...
        if let Some((controls, controls_layout)) = controls {
            ...
            return (Primitive::Group { primitives }, controls_interaction);
        }
